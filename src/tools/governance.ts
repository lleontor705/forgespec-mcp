...
export function registerGovernanceTools(server: McpServer, context: GovernanceToolContext): void {
  ...
  registerIdentityTool<any, any>(server, { verifier, toolName: "authority_manage", description: "Manage bounded authority for enrolled worker handles.", businessSchema: authority, publishedInputSchema: authorityPublishedInput, publishedInputJsonSchema: authorityPublishedJsonSchema, outputSchema: authorityResult, annotations: mutationAnnotations, rejectCallerFields: true, handler: async (input: any, principal) => {
     try { const db = context.database(); const r = input.resource && { kind: input.resource.kind, boardId: input.resource.board_id, ...(input.resource.kind === "task" ? { resourceId: input.resource.task_id } : {}) }; if (input.action === "grant" || input.action === "handoff") return { ok: true, authorities: grantAuthority(db, { actor: actor(principal), granteeActor: target(verifier, input.action === "grant" ? input.grantee_handle : input.to_handle), resource: r!, operations: input.operations, expiresAt: input.expires_at, idempotencyKey: input.idempotency_key }) }; if (input.action === "revoke") { revokeAuthority(db, { actor: actor(principal), boardId: input.board_id, authorityId: input.authority_id, reason: input.reason, idempotencyKey: input.idempotency_key }); return { revoked: true }; } return { ok: true, authorities: queryAuthority(db, { actor: actor(principal), resource: r, operation: input.operation }) }; } catch (e) { return fail(e); }
  }});
  registerIdentityTool<any, any>(server, { verifier, toolName: "event_query", description: "Query audit events bound to the verified worker.", businessSchema: events, outputSchema: eventQueryResult, annotations: readAnnotations, rejectCallerFields: true, handler: async (input: any, principal) => { try { return queryEvents(context.database(), { ...input, actor: actor(principal) }, { cursorSecret: context.cursorSecret }); } catch (e) { return fail(e); } }});
}
